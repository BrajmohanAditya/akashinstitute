...
//   Loader2,
// } from "lucide-react";
// import { useCreateQuizQuestionHook } from "../../hooks/quiz.createQuest.hook";
...
//   const currentSection = sections[currentSectionIdx] || { name: "Category", totalQuestions: 1 };
...
//     if (currentStep > stepNum) return "bg-green-100 text-green-700 font-semibold";
...
//             </div>
//             <div>
//               <h2 className="text-xl font-bold text-slate-900">Create Questions</h2>
//               <div className="flex items-center gap-2 text-sm text-slate-500 mt-1">
//                 <span className="text-indigo-600 font-semibold">Question {currentGlobal} of {globalTotal}</span>
//                 <span className="text-slate-300">|</span>
//                 <span>Category: <span className="font-semibold text-slate-700">{currentSection.name}</span></span>
//                 <span className="text-slate-300">|</span>
//                 <span className="text-slate-400">(#{currentQuestionIdx + 1} in this category)</span>
//               </div>
//             </div>
//           </div>
...
//                const completedInSec = idx < currentSectionIdx ? sec.totalQuestions : idx === currentSectionIdx ? currentQuestionIdx : 0;
...
//                 >
//                   {sec.name}: {completedInSec}/{sec.totalQuestions} {isActive && <span className="ml-1 text-indigo-400">—</span>}
//                 </div>
//                );
//             })}
//           </div>
...
//             <div className={`flex items-center px-6 py-2.5 rounded-xl text-sm transition-all ${getStepIconClass(1)}`}>
//               {getStepIcon(1, HelpCircle)}
//               Question
//             </div>
//             <ChevronRight className="w-4 h-4 text-slate-300" strokeWidth={3} />
//             <div className={`flex items-center px-6 py-2.5 rounded-xl text-sm transition-all ${getStepIconClass(2)}`}>
//               {getStepIcon(2, Layers)}
//               Options
//             </div>
//             <ChevronRight className="w-4 h-4 text-slate-300" strokeWidth={3} />
//             <div className={`flex items-center px-6 py-2.5 rounded-xl text-sm transition-all ${getStepIconClass(3)}`}>
...
//                   <span className="font-bold">Category: {currentSection.name}</span> <span className="text-blue-500 ml-1">(Question {currentQuestionIdx + 1} of {currentSection.totalQuestions} in this category)</span>
//                 </div>
                
//                 <div className="space-y-3">
//                   <label className="block text-sm font-semibold text-slate-700">Question Text <span className="text-red-500">*</span></label>
//                   <textarea
...
//                   </div>
//                   <div className="space-y-3">
//                     <label className="block text-sm font-semibold text-slate-700">Question Image (optional)</label>
//                     <div className="relative h-[46px]">
//                       <input type="file" className="hidden" id="questionImage" />
//                       <label htmlFor="questionImage" className="flex items-center justify-start px-4 w-full h-full border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors text-slate-500 text-sm shadow-sm bg-white">
...
//                     <div key={idx} className="p-5 border border-slate-200 rounded-xl space-y-4 bg-white shadow-sm">
...
//                             <span className="text-xs font-semibold text-slate-600 block">Option image (optional)</span>
//                             <div className="relative">
//                                 <input type="file" className="hidden" id={`optionImage-${idx}`} />
//                                 <label htmlFor={`optionImage-${idx}`} className="flex items-center px-4 py-2.5 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors text-slate-500 text-sm bg-white w-full">
//                                 <Upload className="w-4 h-4 mr-2 text-slate-400" />
//                                 Upload
//                                 </label>
//                             </div>
...
//                   <label className="block text-sm font-bold text-slate-800 mb-4">Select Correct Answer <span className="text-red-500">*</span></label>
...
//                 <div className="space-y-3">
//                   <label className="block text-sm font-semibold text-slate-700">Solution Explanation (Optional)</label>
//                   <textarea
...
//                 <div className="space-y-3">
//                   <label className="block text-sm font-semibold text-slate-700">Solution Image (optional)</label>
//                   <div className="relative h-[46px]">
//                     <input type="file" className="hidden" id="solutionImage" />
//                     <label htmlFor="solutionImage" className="flex items-center justify-start px-4 w-full h-full border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors text-slate-500 text-sm shadow-sm bg-white">
//                       <Upload className="w-4 h-4 mr-2 text-slate-400" />
//                       Upload
//                     </label>
//                   </div>
...
import React, { useState } from "react";
import {
  X, FileQuestion, Layers, BookOpen, Upload,
  ChevronLeft, ChevronRight, Save, HelpCircle, CheckCircle2, Loader2,
} from "lucide-react";
import { useCreateQuizQuestionHook } from "../../hooks/quiz.createQuest.hook";
import { toast } from "sonner";
...
const QuizQuestionAdd = ({ isOpen, onClose, quiz }) => {
  ...
  // State to hold the data the user types into the form
  const [formData, setFormData] = useState({
    questionText: "",
    marks: "",
    questionImage: null,
  ...
  // If the modal isn't supposed to be open, return null to hide it
    // --- Wizard Tracking State ---
  // Tracks which section we are in (0 = the first section)
  const [currentSectionIdx, setCurrentSectionIdx] = useState(0);
  // Tracks which question in that section we are writing (0 = the first question)
  const [currentQuestionIdx, setCurrentQuestionIdx] = useState(0);
  // Tracks which step of the modal we are on (1: Question, 2: Options, 3: Solution)
  const [currentStep, setCurrentStep] = useState(1); 

  // Grab the sections from the quiz prop. (We provide a fallback just in case).
  const sections = quiz?.section || [
    { name: "Reasoning", totalQuestions: 1 },
    { name: "Quant", totalQuestions: 1 }
  ];
  
  // Figure out the current section data based on our index
  const currentSection = sections[currentSectionIdx] || { name: "Category", totalQuestions: 1 };

  // --- Navigation Functions ---
  // Moves us to the next step (e.g., from Question -> Options)
  const handleNext = () => {
    if (currentStep < 3) setCurrentStep(currentStep + 1);
  };

  // Moves us to the previous step (e.g., from Options -> Question)
  const handleBack = () => {
    if (currentStep > 1) setCurrentStep(currentStep - 1);
  };
